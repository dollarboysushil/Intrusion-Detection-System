...
def capture_traffic_isolated(cycle_id, workspace):
    ...
    try:
        ...
        if os.path.exists(pcap_file) and os.path.getsize(pcap_file) > 0:
            logger.info(f"[{cycle_id}] Capture successful: {pcap_file} ({os.path.getsize(pcap_file)} bytes)")
            return pcap_file
        else:
            logger.warning(f"[{cycle_id}] Capture resulted in empty or missing file")
            return None
            
    except subprocess.TimeoutExpired:
        logger.error(f"[{cycle_id}] Capture timeout - killing tshark process")
        subprocess.run(["taskkill", "/F", "/IM", "tshark.exe"], capture_output=True, shell=True)
        return None
...
def convert_pcap_isolated(pcap_file, workspace, cycle_id):
    ...
    try:
        ...
        if csv_file and os.path.exists(csv_file):
            ...
            logger.info(f"[{cycle_id}] CSV conversion successful: {csv_file} ({row_count} flows)")
            return csv_file
        else:
            logger.warning(f"[{cycle_id}] No CSV file generated from {pcap_file}")
            return None
...
def process_predictions_isolated(csv_file, cycle_id):
    ...
    try:
        ...
        if total_flows > MAX_FLOWS_PER_CYCLE:
            logger.info(f"[{cycle_id}] Limiting flows from {total_flows} to {MAX_FLOWS_PER_CYCLE}")
        ...
        logger.info(f"[{cycle_id}] Sending {len(rows)} flows for prediction (from {total_flows} total)")
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(future_to_row, timeout=PREDICTION_TIMEOUT):
                ...
                if time.time() - start_time > PREDICTION_TIMEOUT:
                    logger.warning(f"[{cycle_id}] Prediction timeout reached, cancelling remaining requests")
                ...
                try:
                    result = future.result(timeout=1)  # Quick timeout for individual results
                    ...
                    if completed_requests % 25 == 0:
                        logger.info(f"[{cycle_id}] Progress: {completed_requests}/{len(rows)} requests completed")
                        
                except Exception as e:
                    predictions['errors'] += 1
                    logger.debug(f"[{cycle_id}] Error processing row {row_index}: {e}")
...
def log_cycle_summary(cycle_id, predictions):
    ...
    if original_total != total:
        logger.info(f"[{cycle_id}]   Original flows: {original_total} (sampled: {total})")
    else:
    ...
    if attack_percentage > 10:  # Threshold for alert
        logger.warning(f"[{cycle_id}] [!] HIGH ATTACK TRAFFIC DETECTED: {attack_percentage:.1f}%")
    else:
        ...
        while not shutdown_event.is_set():
            try:
                ...
                if success:
                    ...
                    logger.warning(f"[FAIL] Cycle failed ({consecutive_failures}/{max_consecutive_failures})")
                    
                    if consecutive_failures >= max_consecutive_failures:
                        logger.error("Too many consecutive failures - performing system reset")
                        # Emergency reset
