...
# Configuration
INTERFACE = "Ethernet"  # Will be auto-detected if this fails
...
def validate_interface(interface_name):
    """Validate if an interface is available"""
    interfaces = get_available_interfaces()
    return any(interface_name.lower() in interface.lower() for interface in interfaces)
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for line in result.stdout.strip().split('\n'):
                if line.strip():
                    # Parse format: "1. \Device\NPF_{GUID} (Network adapter name)"
                    parts = line.split(' ', 1)
                    if len(parts) > 1:
...
def find_best_interface():
    """Find the best network interface to use"""
    interfaces = get_available_interfaces()
    ...
    for i, interface in enumerate(interfaces, 1):
        logger.info(f"  {i}. {interface}")
    
    # Try to find ethernet-like interfaces first
    ethernet_keywords = ['ethernet', 'local area connection', 'wi-fi', 'wireless', 'network']
    
    for interface in interfaces:
        interface_lower = interface.lower()
        for keyword in ethernet_keywords:
            if keyword in interface_lower:
                logger.info(f"Selected interface: {interface}")
                return interface
    
    # If no ethernet-like interface found, use the first one
    if interfaces:
        selected = interfaces[0]
        logger.info(f"Using first available interface: {selected}")
        return selected
...
def capture_traffic(capture_id, workspace):
    ...
    # If the configured interface fails, try to auto-detect
    if not validate_interface(interface_to_use):
        logger.warning(f"[CAPTURE] Interface '{interface_to_use}' not available, auto-detecting...")
        interface_to_use = find_best_interface()
        if not interface_to_use:
            logger.error(f"[CAPTURE] {capture_id} no valid interface found")
            return None
...
def main():
    ...
    # Auto-detect and validate network interface
    global INTERFACE
    if not validate_interface(INTERFACE):
        logger.warning(f"Configured interface '{INTERFACE}' not found, auto-detecting...")
        INTERFACE = find_best_interface()
        if not INTERFACE:
            logger.error("No valid network interface found. Please check tshark installation and network adapters.")
            return
